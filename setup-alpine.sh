...
# ==============================================================================
#   IoT Server Unified Setup Script (Final Version with All Fixes)
# ==============================================================================
...
generate_and_publish_uri() {
    ...
    if [ -n "$URI" ]; then
        ...
        if [ -n "$BOT_TOKEN" ] && [ -n "$CHANNEL_ID" ]; then
            ...
            # استفاده از تایم‌اوت ۱۰ ثانیه‌ای برای جلوگیری از گیر کردن اسکریپت
...
# فقط اگر لینک V2Ray در کانفیگ وجود داشته باشد، Xray را نصب می‌کند
if [ -n "$V2RAY_LINK" ]; then
    curl -L -o xray.zip https://github.com/XTLS/Xray-core/releases/latest/download/Xray-linux-64.zip
    unzip -o xray.zip -d /usr/local/bin/ && rm xray.zip && chmod +x /usr/local/bin/xray
fi

# پیکربندی و اجرای V2Ray/Xray (فقط اگر لینک آن ارائه شده باشد)
if [ -n "$V2RAY_LINK" ]; then
...
EOF
    ...
    # --- دستور اصلاح‌شده و صحیح برای اجرای Xray با PM2 ---
    # ما از -- برای جدا کردن آرگومان‌های Xray از آرگومان‌های PM2 استفاده می‌کنیم
...
# فراخوانی تابع تولید URI به عنوان یکی از آخرین مراحل
generate_and_publish_uri
